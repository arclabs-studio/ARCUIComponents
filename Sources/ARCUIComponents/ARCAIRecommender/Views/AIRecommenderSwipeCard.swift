...
struct AIRecommenderSwipeCard<Item: AIRecommenderItem>: View {
    // MARK: - Properties
    ...
    private var bookmarkHeight: CGFloat = 48
    @ScaledMetric(relativeTo: .body)
    private var quoteBarWidth: CGFloat = 3
    @ScaledMetric(relativeTo: .body)
    private var heroHeight: CGFloat = 180
                ...
                HStack(spacing: .arcSpacingXSmall) {
                    ForEach(Array(tags.enumerated()), id: \.offset) { index, tag in
                        ARCChip(
                            ...
                            configuration: ARCChipConfiguration(
                                size: .small,
...
#if os(iOS)
@available(iOS 17.0, macOS 14.0, *)
private struct PreviewItem: AIRecommenderItem {
    let id: UUID
    let title: String
    var subtitle: String?
    var rating: Double?
