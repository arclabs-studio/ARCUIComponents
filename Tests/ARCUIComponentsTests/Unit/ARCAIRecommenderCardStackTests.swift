...
struct ARCAIRecommenderCardStackConfigurationTests {
    @Test("default_preset_enablesCardStack")
    func default_preset_enablesCardStack() {
        let config = ARCAIRecommenderConfiguration.default

        #expect(config.useCardStack == true)
    }

    @Test("list_preset_disablesCardStack")
    func list_preset_disablesCardStack() {
        let config = ARCAIRecommenderConfiguration.list

        #expect(config.useCardStack == false)
    }

    @Test("default_hasCorrectCardStackDepth")
    func default_hasCorrectCardStackDepth() {
        let config = ARCAIRecommenderConfiguration.default

        #expect(config.cardStackDepth == 3)
    }

    @Test("default_hasCorrectSwipeThreshold")
    func default_hasCorrectSwipeThreshold() {
        let config = ARCAIRecommenderConfiguration.default

        #expect(config.swipeThreshold == 0.3)
    }

    @Test("default_hasCorrectMaxSwipeRotation")
    func default_hasCorrectMaxSwipeRotation() {
        let config = ARCAIRecommenderConfiguration.default

        #expect(config.maxSwipeRotation == 8.0)
    }

    @Test("default_showsCardIndicator")
    func default_showsCardIndicator() {
        let config = ARCAIRecommenderConfiguration.default

        #expect(config.showCardIndicator == true)
    }

    @Test("default_hasCorrectBookmarkIcons")
    func default_hasCorrectBookmarkIcons() {
        let config = ARCAIRecommenderConfiguration.default
    ...
    @Test("default_hasAllViewedText")
    func default_hasAllViewedText() {
        let config = ARCAIRecommenderConfiguration.default

        #expect(!config.allViewedText.isEmpty)
    }

    @Test("init_withCustomSwipeThreshold_setsCorrectly")
    func init_withCustomSwipeThreshold_setsCorrectly() {
        let config = ARCAIRecommenderConfiguration(swipeThreshold: 0.5)

        #expect(config.swipeThreshold == 0.5)
    }

    @Test("init_withCustomCardStackDepth_setsCorrectly")
    func init_withCustomCardStackDepth_setsCorrectly() {
        let config = ARCAIRecommenderConfiguration(cardStackDepth: 5)

        #expect(config.cardStackDepth == 5)
    }

    @Test("init_withCustomBookmarkIcons_setsCorrectly")
    func init_withCustomBookmarkIcons_setsCorrectly() {
    ...
    @Test("minimal_preset_enablesCardStack")
    func minimal_preset_enablesCardStack() {
        let config = ARCAIRecommenderConfiguration.minimal

        #expect(config.useCardStack == true)
    }

    @Test("compact_preset_enablesCardStack")
    func compact_preset_enablesCardStack() {
...
struct AIRecommenderItemProtocolTests {
    @Test("defaultImplementation_location_returnsNil")
    func defaultImplementation_location_returnsNil() {
    ...
    @Test("defaultImplementation_highlightDetail_returnsNil")
    func defaultImplementation_highlightDetail_returnsNil() {
        struct MinimalItem: AIRecommenderItem {
            let id: UUID
            let title: String
        }
    ...
    @Test("customImplementation_location_returnsValue")
    func customImplementation_location_returnsValue() {
        let item = MockCardItem(
            id: UUID(),
            title: "Test",
...
struct AIRecommenderImageSourceHeroTests {
    @Test("system_imageSource_createsHeroView")
    func system_imageSource_createsHeroView() {
    ...
    @Test("placeholder_imageSource_createsHeroView")
    func placeholder_imageSource_createsHeroView() {
        let source = AIRecommenderImageSource.placeholder(text: "LT", colors: [.orange, .red])

        _ = source.heroImageView(height: 180)
    }

    @Test("url_imageSource_createsHeroView")
    func url_imageSource_createsHeroView() throws {
        let source = try AIRecommenderImageSource.url(#require(URL(string: "https://example.com/image.jpg")))

        _ = source.heroImageView(height: 200)
    }

    @Test("image_imageSource_createsHeroView")
    func image_imageSource_createsHeroView() {
